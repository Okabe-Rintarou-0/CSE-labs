...
//    std::cout << "rd: " << (int)buf[1]<<std::endl;
}

void
disk::write_block(blockid_t id, const char *buf)
{
//    std::cout << "before: " << (int)blocks[id][1]<<std::endl;
//    std::cout << "try wt: "<< (int)buf[1] << std::endl;
//    auto t = blocks[id];
    memcpy((char *) blocks[id], buf, BLOCK_SIZE);
//    std::cout << "after: " << (int)blocks[id][1]<<std::endl;
}
...
inode_manager::alloc_inode(uint32_t type)
{
  ...
  {
    uint32_t iid = IBLOCK(i, this->bm->sb.nblocks);
//    std::cout << "iid: " << iid << std::endl;
    if (bm->check_free_and_alloc(iid))
    {
      inode *_inode = (inode *) malloc(sizeof(inode));
      _inode->type = type;
      _inode->size = 0;
      put_inode(i, _inode);
      free(_inode);
...
inode_manager::free_inode(uint32_t inum) {
  ...
  if (i == nullptr) return;
  if (!bm->check_free_and_free(inode_block_id))
  {
...
//  std::cout << "file size: " << *size << std::endl;
  ...
  unsigned int n_direct_blocks = direct_block_size % BLOCK_SIZE ? direct_block_size / BLOCK_SIZE + 1 : direct_block_size / BLOCK_SIZE;
...
//    std::cout << "Read indirect_block_id: " << indirect_block_id << std::endl;
    ...
    n_direct_blocks = indirect_block_size % BLOCK_SIZE ? indirect_block_size / BLOCK_SIZE + 1 : indirect_block_size / BLOCK_SIZE;
...
//  std::cout << "Try write: " << test << std::endl << "to: " << inum << std::endl;
  inode *_inode = get_inode(inum);
  // check validation
  if (_inode == nullptr)
    return;
  ...
  unsigned n_direct_blocks = direct_size % BLOCK_SIZE ? direct_size / BLOCK_SIZE + 1 : direct_size / BLOCK_SIZE;
...
//  std::cout << "wt indir size: " << indirect_size << std::endl;
  if (indirect_size > 0)
  {
    // indirect block => many direct blocks
    unsigned indirect_block_id = this->bm->alloc_block();
